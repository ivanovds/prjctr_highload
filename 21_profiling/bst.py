...
def from_array(val):
    # val.sort()
    return sorted_array_to_bst(val)
    
def sorted_array_to_bst(val):
    if not len(val):
        return None

    mid_val = len(val) // 2
    node = BSTNode(val[mid_val])
    node.left = sorted_array_to_bst(val[:mid_val])
    node.right = sorted_array_to_bst(val[mid_val+1:])
