...
def main():
    ...
    while search != 'q':
        ...
        if len(search) > 7:
            completion = True
            data = '{ "suggest": { "country-suggest" : { "prefix" : "%s", "completion" : { "field" : ' \
                   '"Name.completion", "fuzzy": { "fuzziness": 3 } } } } }' % search

        else:
            data = '{ "query": { "match": { "Name.standard": "%s" } } }' % search

        response = requests.get('http://localhost:9200/countries/_search?pretty',
                                data=data,
                                headers=headers
                                )
